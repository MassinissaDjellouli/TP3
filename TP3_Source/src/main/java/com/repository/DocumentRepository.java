...
public interface DocumentRepository extends JpaRepository<Documents,Integer> {
    public Optional<List<Documents>> findAllByTitreContaining(@Param("titre") String titre);
    public Optional<List<Documents>> findAllByAuteur(@Param("auteur") String auteur);
    public Optional<List<Documents>> findAllByAnneeDePublication(@Param("anne") int anne);
    @Query("select l from Livre l where l.genre = :genre")
    public Optional<List<Documents>> findAllByGenre(@Param("genre") Genres genre);
